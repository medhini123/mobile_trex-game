...
function setup() {
  ...
  trex = createSprite(40, height-20, 80, 80);
...
function draw() {
  ...
  if (gamestate === play) {
    ...
    if ((touches.length>0||keyDown("UP_ARROW")) && trex.y >= height-6) {
      trex.velocityY = -10;
      jump.play();
      touches=[]
    } else if ((touches.length>0||keyDown("space")) && trex.y >= 171) {
      trex.velocityY = -10;
      jump.play();
      touches=[]
    }
...
function createclouds() {
  if (frameCount % 80 === 0) {
    clouds = createSprite(600, height-50, 100, 20);
...
function createobstacle() {
  if (frameCount % 50 === 0) {
    obstacle = createSprite(width/2, height-20, 10, 20);
